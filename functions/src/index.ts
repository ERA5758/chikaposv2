...
export const onTopUpRequestUpdate = onDocumentUpdated("topUpRequests/{requestId}", async (event) => {
  ...
  // Sync the status update back to the store's subcollection
  try {
    const historyRef = db.collection('stores').doc(storeId).collection('topUpRequests').doc(requestId);
    await historyRef.update({ status: after.status, processedAt: FieldValue.serverTimestamp() });
    logger.info(`Synced status update for request ${requestId} to store ${storeId}`);
  } catch (syncError) {
    logger.error(`Failed to sync status for request ${requestId} to store ${storeId}:`, syncError);
